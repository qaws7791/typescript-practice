...
const isCustomer = (user: User): boolean => user.role === "customer";
...
const getBeverages = (user: User): Beverage[] => (!user ? [] : beverages);

const findBeverage = (beverageName: string): Beverage | undefined =>
  beverages.find((beverage) => beverage.name === beverageName);
